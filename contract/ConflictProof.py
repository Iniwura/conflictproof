...
STATUS_CLASSES = (
    "OK", "TRANSIENT", "REDIRECT", "NOT_FOUND", "TERMINAL_HTTP",
    "REJECTED_MEDIA", "NON_BYTE_BODY", "OVERSIZED_BODY", "INVALID_UTF8",
    "INVALID_TEXT", "EMPTY_TEXT", "OVERSIZED_TEXT",
)

OBSERVATION_KEYS = (
    "role", "role_index", "url", "status_class", "available",
    "media_accepted", "redirect_blocked", "content_digest",
)

COMMON_KEYS = (
    "schema_version", "case_id", "state", "retry_count", "case_digest",
    "source_observations", "observation_digest",
)

RETRY_KEYS = COMMON_KEYS

FINAL_KEYS = COMMON_KEYS + (
    "status", "finalized_at", "evaluation_digest", "result_digest",
)
...
def _normalize_hostname(host):
    ...
    if host == "localhost" or any(
        ...
        for suffix in (".localhost", ".local", ".internal", ".lan", ".invalid", ".test")
    ):
...
def _normalize_url(value):
    ...
    if any(
        ...
        _fail(
            "INVALID_URL",
            "ASCII URL without unsafe characters required",
        )
...
def _validate_case(raw):
    ...
    result = {
        ...
        "conflict_claim": _text(
            value["conflict_claim"],
            MAX_CONFLICT_CLAIM,
            "INVALID_CASE",
            "conflict_claim",
        ),
...
def _case_payload(case):
    return {
        ...
        "conflict_claim": case["conflict_claim"],
        "rule_a": case["rule_a"],
        "rule_b": case["rule_b"],
    }
...
def _accepted_media(media):
    if media.startswith("image/") or media in (
        "image/svg+xml",
        "application/svg+xml",
    ):
        return False
...
def _context(case, rule_a_text, rule_b_text):
    value = {
        ...
        "conflict_claim": case["conflict_claim"],
        "rule_a": {
            "statement": case["rule_a"]["statement"],
            "label": case["rule_a"]["label"],
            "source_url": case["rule_a"]["source_url"],
            "content": rule_a_text,
        },
        "rule_b": {
            "statement": case["rule_b"]["statement"],
            "label": case["rule_b"]["label"],
            "source_url": case["rule_b"]["source_url"],
            "content": rule_b_text,
        },
    }
...
def _proposal(
    ...
    rule_a_observation, rule_a_text = _fetch(
        RULE_A,
        0,
        case["rule_a"]["source_url"],
    )

    rule_b_observation, rule_b_text = _fetch(
        RULE_B,
        1,
        case["rule_b"]["source_url"],
    )
    ...
    if (
        transient
        or not rule_a_observation["available"]
        or not rule_b_observation["available"]
    ):
        status = UNRESOLVED
    else:
    ...
    final["evaluation_digest"] = _evaluation_digest(
        case,
        observations,
        retry_count,
        observation_digest,
    ...
    final["result_digest"] = _result_digest(
        case,
        observations,
        observation_digest,
        status,
...
class ConflictProof(gl.contract.Contract):
    ...
    def create_case(self, case_json: str) -> str:
        ...
        self.case_count = u256(
            counter
        )

        creator_count = int(
            self.creator_case_count.get(
                creator,
                u256(0),
            )
        ) + 1

        self.creator_case_count[
            creator
        ] = u256(creator_count)

        self.creator_case_id[
            creator + ":" + str(creator_count)
        ] = case_id
    ...
    def evaluate(self, case_id: str) -> None:
        ...
        if self.evaluation_records.get(
            case_id,
            "",
        ) != "":
            _fail(
    ...
    def retry_evaluation(self, case_id: str) -> None:
        ...
        raw = self.evaluation_records.get(
            case_id,
            "",
        )
    ...
    def get_creator_case_id(self, creator: str, index: int) -> str:
        ...
        if index > count:
            _fail(
                "INVALID_INDEX",
                "index exceeds creator count",
            )

        return self.creator_case_id.get(
            creator + ":" + str(index),
            "",
        )
